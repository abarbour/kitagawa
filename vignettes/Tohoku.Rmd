...
`r signif(coef(lm(output ~ input - 1, as.data.frame(toh.dat))), 3)` GPa/$\epsilon$, 
...
```{r, echo=TRUE}
class(toh.dat)
toh.cs <- psd::pspectrum(toh.dat, ntap.init=k, niter = 1)
class(toh.cs)
str(toh.cs)
```
...
f <- as.vector(toh.cs[['freq']]) 
lf <- log10(f)
p <- 1/f
lp <- log10(p)

Coh <- toh.cs[['coh']]
Phi <- as.vector(toh.cs[['phase']]) # wrapped phase -- (??) is phase lag positive? [ ]

suppressPackageStartupMessages(can.unwrap <- require(signal))
if (can.unwrap){
  Phi <- signal::unwrap(Phi)
}
G <- Mod(toh.cs[['transfer']]) # Gain
G <- Coh / G # Must do this to get back to ~2.5 GPa/strain -- (??) Is the definition different in psd? [ ]
K <- toh.cs[['taper']]
k <- as.numeric(K)
G.err <- sqrt((1 - Coh) / k)
...
```{r, echo=TRUE}
csd <- data.frame(f, p, lf, lp, Coh, k, G, G.err, Phi = Phi * 180 / pi)
...
layout(matrix(1:3), heights=c(1.5,2,1.5))
par(mar=c(1, 4, 0.0, 4), oma=c(4,1,2,1), cex=0.8, las=1)
...
mtext('Tapers', side=1, font=3, line=-2.5, adj=0.3)
axis(2, at=seq(0,1,by=0.2))
axis(1, col=NA, col.ticks=1, labels=FALSE)
par(new=TRUE)
plot(K, xaxs='i', yaxs='i', axes=FALSE, ylim=c(0,1100),xlab='', ylab='')
...
with(csd.f, {
  ...
  polygon(c(f, rev(f)), c(Upper, pmax(ylim[1], rev(G - nsig*G.err))), col='lightcyan', border='lightgrey')
  lines(f, G)
})
...
plot(Phi-180 ~ f, csd.f, 
     log='x',
     type='l', #col='lightgrey', 
     xlab='', ylab="Degrees (lag)",
     xaxs='i', frame=FALSE,
     yaxs='i', 
     yaxt='n')
yat <- 180*(-2:2)/2
abline(h=yat, lty=3)
mtext(sprintf('Relative Phase\n%s',ifelse(can.unwrap," (Unwrapped)","")), font=2, line=-3.0, adj=0.3)
axis(2)
