...
k <- 2*130
class(toh.dat)
toh.cs <- psd::pspectrum(toh.dat, ntap.init=k, niter = 0)
class(toh.cs)
str(toh.cs)
```
...
with(toh.cs,{
  ...
  par(mar=c(2,3,1,1), mgp=c(2,0.5,0), tcl=0.5, oma=c(2,0,0,0))
  f <- freq
  plot(f, coh, type='l', log='x', ylim=c(0,1), ylab="Coherence")
  plot(f, signal::unwrap(phase[,1]), type='l', log='xy', ylab="Phase, rad")
  abline(h=pi)
  plot(f, Mod(transfer), type='l', log='xy', ylab="Gain", xlab="Frequency, Hz")
})
```

### Evaluating the Response: Coherence, Admittance, and Phase
...
all.equal(Coh, as.vector(toh.cs[['coh']])) # something awry in this definitions above
G <- abs(sqrt(Coh * S22 / S11))
all.equal(G, Mod(S12))
Phi <- atan2(x = Re(S12), y = Im(S12))
Phi2 <- Arg(S12)
all.equal(Phi, Phi2)
all.equal(Phi, -as.vector(toh.cs[['phase']]))
```
