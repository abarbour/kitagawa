...
Q <- 10**seq(-5,2,by=0.05) 					# == z**2 omega / 2 D
lQ <- log10(Q)
omega <- omega_norm(Q, z, D., invert=TRUE)		# == Q * 2 * Diffus / z**2
...
#
rhog <- 9.81*1000
Ku. <- 40e9	# Bulk modulus
B. <- 0.5	# Skemptons coeff
WR <- well_response(omega, T.=T., S.=S., Vw.=Vw., Rs.=Rs.,	Ku.=Ku., B.=B., Avs=1, Aw=1) #4.8)
rsp <- WR[["Response"]]
#
# Amplitude
kGain <- rsp[,2]*rhog/Ku./B.
# Phase
kPhs <- rsp[,3] # will wrap to -pi/pi
...
plot(lQ, log10(kGain), type="l", #ylim=c(0, 1.15),
     xaxt="n", #yaxs="i",
     lwd=2,
     ylab="", #" (cm/nanostrain)", 
...
wrsp <- open_well_response(omega, T.=T., S.=S., model = "cooper")
...
mtext("(a) Gain", adj=0)
par(mar=c(4,4,1.1,2))
plot(lQ, cPhs*180/pi, type="l", lty=3, #ylim=c(-190, -130), 
	 xaxt="n",
     ylab="degrees", xlab=expression("Dimensionless frequency," ~ Q ==z^2 * omega / 2 * D  ))
abline(h=-180, col="grey")
lines(lQ, cuPhs*180/pi, type="l", lwd=2)
log10_ticks()
mtext("(b) Phase", adj=0)
#text(1.4, -140, "phase crossings", font=3, cex=0.8)
@
...
wrsp <- open_well_response(omega, T.=T., S.=S., model = "hsieh")
crsp <- wrsp[["Response"]][,2]	# complex response
# invalid above Q ~ 30
#crsp[Q>=30] <- NA
#
...
wrsp <- open_well_response(omega, T.=T., S.=S., model = "liu")
crsp <- wrsp[["Response"]][,2]	# complex response
# invalid above Q ~ 30
#crsp[Q>=30] <- NA
#
...
wrsp <- open_well_response(omega, T.=T., S.=S., z=z, model = "rojstaczer", as.pressure=FALSE)
crsp <- wrsp[["Response"]][,2]		# complex response
#
# Amplitude
As <- 0.05 # cm/nE
...
par(mar=c(4,4,1.1,2))
plot(lQ, rPhs*180/pi, type="l", lty=3, ylim=c(-180, -130), 
	 xaxt="n",
     ylab="degrees", xlab=expression("Dimensionless frequency," ~ Q ==z^2 * omega / 2 * D  ))
abline(h=-180, col="grey")
...
par(mar=c(4,4,1.1,2))
plot(lQ, kPhs*180/pi, lty=3, lwd=1.5, col=Set1[1],
	type="l", ylim=185*c(-1,1), 
	yaxt="n",
	xaxt="n",
    ylab="degrees", 
...
lines(lQ, rPhs*180/pi, lty=3, lwd=1.5, col=Set1[2])
# unwrapped phase
lines(lQ, kuPhs*180/pi, lwd=2, col=Set1[1])
lines(lQ, ruPhs*180/pi, lwd=2, col=Set1[2])
log10_ticks()
mtext("(b) Phase", adj=0)
@
...
# cooper, liu, hsieh
plot(lQ, log10(hGain/rhog), col=Set1[3],
	 type="l", 
     xaxt="n", 
     lwd=2,
     ylab="",
     xlab="",
     main="")
lines(lQ, log10(cGain/rhog), lwd=2, col=Set1[1])
lines(lQ, log10(lGain/rhog), lwd=2, col=Set1[2])
legend("bottomright", 
...
par(mar=c(4,4,1.1,2))
plot(lQ, cPhs*180/pi, lty=3, lwd=1.5, col=Set1[1],
	type="l", ylim=185*c(-1,1), 
	yaxt="n",
	xaxt="n",
    ylab="degrees", 
...
lbls[seq_along(lbls)%%2==0] <- ""
axis(2, at=ats, labels=lbls)
lines(lQ, lPhs*180/pi, lty=3, lwd=1.5, col=Set1[2])
lines(lQ, hPhs*180/pi, lty=3, lwd=1.5, col=Set1[3])
# unwrapped phase
lines(lQ, cuPhs*180/pi, lwd=2, col=Set1[1])
lines(lQ, luPhs*180/pi, lwd=2, col=Set1[2])
lines(lQ, huPhs*180/pi, lwd=2, col=Set1[3])
log10_ticks()
mtext("(b) Phase", adj=0)
@
